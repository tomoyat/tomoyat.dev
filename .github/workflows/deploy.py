...
def object_cache_setting(path):
    ...
    if os.fspath(path).startswith('page-data/'):
        return "public, max-age=0, must-revalidate"

    if path.name in ['app-data.json', 'page-data.json', 'sw.js']:
        return "public, max-age=0, must-revalidate"

    if path.name in ['favicon.ico']:
        return "public, max-age=86400"

    if os.fspath(path).startswith('static/'):
